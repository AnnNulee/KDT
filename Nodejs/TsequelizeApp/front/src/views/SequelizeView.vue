            ...
            try{
                ...
                // 사용자 등록창 초기화
                    // this.newUser = {name : '', age : null, married : false}
                            ...
                            // 서버로부터 오는 응답인, res.data를 받아주는 공간(this.users)과 front에서 axios.POST 요청을 위해 template에서 정보를 받아오는 데이터(this.newUser)의 포멧(속성값)이 서로 다르다. 
                            // 이건 front에서 axios.get 요청 req로서 서버(server - routes)에 들어갈때 req.body 속성에 데이터를 담아 보낸다.
                            // req.body에 들어간 정보는, app.js에서 import해준 express.json() 바디파싱함수가 데이터를 파싱하여 req.body.속성 형식으로 사용할 수 있게 해준다.
                            // 그러면 DB에 있는 실제 테이블(컬렉션)의 속성과, front의 vue-data에서 정의해준 속성을 맵핑하여 값을 전달할 수 있다. 
